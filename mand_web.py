...
# --- 1. API AYARI ---
# Groq üzerinden Llama 3.1 kullanıyoruz
client = OpenAI(
...
# Sayfa yenilendiğinde verilerin silinmemesi için
...
# KRİTİK: Sonsuz döngüyü engelleyen ID takibi
...
with st.sidebar:
    ...
    <div class="stats-box">
        ...
        <span style='color:#fbbf24;'>⚠️ <b>Mistakes:</b> {st.session_state.stats['mistakes']}</span>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    st.title("MAND Bot v8.3 Web Edition")
    st.write(f"Level: **{st.session_state.level}** | User: **{st.session_state.user_name}**")

# Sohbet Geçmişini Bas
for m in st.session_state.messages:
    with st.chat_message(m["role"]): st.markdown(m["content"])

# --- 7. GİRİŞ VE DÖNGÜ KONTROLÜ ---
st.divider()
# MİKROFON TUŞU
...
# SADECE YENİ BİR SES GELDİĞİNDE İŞLE (DÖNGÜYÜ BU ÇÖZER)
if audio_data:
